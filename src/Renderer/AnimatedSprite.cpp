...
namespace Renderer {
    ...
    AnimatedSprite::AnimatedSprite(shared_ptr<Texture2D> pTexture,
        string initialSubTexture,
        shared_ptr<ShaderProgram> pShaderProgram,
        const glm::vec2& position,
        const glm::vec2& size,
        const float rotation)
        : Sprite(move(pTexture), move(initialSubTexture), move(pShaderProgram), position, size, rotation)
    {
        m_pCurrentAnimationDurations = m_statesMap.end();
    }

    void AnimatedSprite::insertState(string state, vector<pair<string, uint64_t>> subTexturesDuration)
    {
        m_statesMap.emplace(move(state), move(subTexturesDuration));
    }

    void AnimatedSprite::setState(const string& newState)
    {
        auto it = m_statesMap.find(newState);
        if (it == m_statesMap.end())
        {
            cout << "Can't find animation state: " << newState << endl;
            return;
        }
